...
| `GET /content/json/{cid}` | Same bytes with `Content-Type: application/json; charset=utf-8` and inline disposition. |
...
| `GET` | `/content/json/{cid}` | Same object as JSON with inline disposition (browser-friendly, CDN-cacheable) |
...
| `GET /content/json/{cid}` | Same | Typed response for JSON; same rationale |
