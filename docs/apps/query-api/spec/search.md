...
4. **`/search`:** Per `object_id`, rank = max of: exact `value_text_normalized = lower(trim(q))` (boost 1000), `ts_rank_cd` on FTS hits, id substring boost (500 when id-shaped). Collapse with `DISTINCT ON (COALESCE(meta_group_id, object_id))` keeping highest rank then `weight`; final `ORDER BY rank DESC, weight DESC NULLS LAST`, `LIMIT :limit`. Then load aggregates, resolve, project.
