...
- **`UserHeroCoverImage`** (profile hero background): same raw → Hive `0x0` chain; hides the image on terminal failure so the band background stays.
- Some hosts (e.g. **`img.3speakcontent.co`**, **`steemitimages.com`**) can fail the optimizer’s **server-side** `fetch` (DNS / network / 403 via proxy). URLs containing **`/ipfs-gateway/content/image/`** are skipped too: a failed optimizer fetch makes `UserAvatar` fall through to the Hive CDN, which does not have that gateway image. Use **`shouldUnoptimizeRemoteImage(src)`** from `@/shared/presentation` and pass **`unoptimized`** to `Image` for those URLs so the browser loads the asset directly; pair with **`onError`** where a visible fallback is needed (e.g. feed preview media, `ObjectThumbnail`).
