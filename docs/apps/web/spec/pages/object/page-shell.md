...
|-----------|--------------|
| Standard object (`objectTypeHasDetailsTab`) + **Details** landing (clean `/object/:id`, no `?path=`) + **view** | Left-rail **Details** blocks, then up to 5 reviews, then the view-mode right rail (Related / Similar / Add-On / field references / experts / followers, each with Show more when the section has items). Nested menu/description in center is **hidden** on mobile (shown at `lg+` only). |
| Same + **edit** | Left-rail **Details** only (edit `+` / grouped sections). No reviews or right-rail lists; center hidden on mobile. |
| Standard + other primary tab or `?path=` drill-down | Center tab content only (unchanged). Right-rail lists stay desktop-only. |
...
| Social links (left rail) | [routes/social-links.md](routes/social-links.md) |
