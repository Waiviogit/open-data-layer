...
DM rows, the open-thread header, and the About roster all read the same query-api fields (`peer_avatar_url`, member `avatar_url`) from `avatarUrlFromJoinedAccountRow` (`posting_json_metadata`, then `json_metadata`, then `profile_image`). Search uses that function too. A `?peer=` draft has no channel row yet: the empty thread keeps the search hit's `profile_image` until the first broadcast, then the channel payload replaces it. When the URL is null, `UserAvatar` falls back to the sized Hive CDN image, then the placeholder. First-party `/ipfs-gateway/content/image/` avatars skip the Next image optimizer so a failed optimizer fetch does not replace them with the Hive CDN.
