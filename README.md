...
After `.env` is created, edit it at least for production: **`DOMAIN`**, **`CERTBOT_EMAIL`**, **`JWT_SECRET`**, **`POSTGRES_PASSWORD`**, and **`AUTH_JWT_SECRET`** (if used). The stack serves HTTPS via `jonasal/nginx-certbot`. **Portainer CE** is at **`https://127.0.0.1:9443`** on the VPS (loopback-only; **`ssh -L 9443:127.0.0.1:9443`** from your machine — see [docs/deployment/portainer.md](docs/deployment/portainer.md)). The **`stack-watchdog`** service in the `apps` compose file periodically pulls GHCR images and recreates app containers; you can still run compose manually or use Portainer. See [docs/apps/stack-watchdog/spec/overview.md](docs/apps/stack-watchdog/spec/overview.md). More on Compose files: [docs/getting-started.md](docs/getting-started.md).
