...
- **Colors:** use semantic utilities (`bg-bg`, `text-fg`, `text-fg-secondary`, `text-fg-tertiary`, `text-muted`, `text-link`, `border-border`, `bg-accent`, `text-accent-fg`, `bg-surface`, extended tokens such as `border-border-strong`, `bg-surface-alt`, `bg-surface-raised`, `text-heading`, `bg-code-bg`, `bg-nav-bg`, `text-nav-fg`). Do **not** embed raw `#…` / `rgb()` / `rgba()` in component `className` or inline styles except where documented (e.g. temporary scaffold CSS).
- **Typography:** root uses `var(--font-body)` from `global.css`; prefer `font-body`, `font-display`, or `font-editorial` when a component needs an explicit stack. Use `tracking-display`, `tracking-body`, `tracking-caption` for letter-spacing and `leading-display`, `leading-body` for line-height so rhythm is theme-controlled.
