...
- **Colors:** use semantic utilities (`bg-bg`, `text-fg`, `text-fg-secondary`, `text-fg-tertiary`, `text-muted`, `text-link`, `border-border`, `bg-accent`, `text-accent-fg`, `bg-secondary`, `text-secondary-fg`, `bg-tertiary`, `text-tertiary-fg`, `bg-error`, `text-error`, `border-error`, `outline-focus`, `bg-surface`, extended tokens such as `border-border-strong`, `bg-surface-alt`, `bg-surface-raised`, `text-heading`, `bg-code-bg`, `bg-nav-bg`, `text-nav-fg`). Do **not** embed raw `#…` / `rgb()` / `rgba()` in component `className` or inline styles except where documented (e.g. temporary scaffold CSS).
- **Typography:** root uses `var(--font-body)` from `global.css`; prefer `font-body`, `font-display`, `font-editorial`, or `font-label` when a component needs an explicit stack. Use `text-display`, `text-section`, `text-body`, `text-caption`, `text-micro` for theme-controlled sizes; `font-weight-display`, `font-weight-body`, `font-weight-label`, `font-weight-strong` for weights; `tracking-display`, `tracking-body`, `tracking-caption` for letter-spacing; `leading-display`, `leading-body` for line-height.
- **Radius and elevation:** prefer token-backed utilities (`rounded-btn`, `rounded-card`, `rounded-card-lg`, `rounded-pill`, `rounded-circle`, `shadow-card`, `shadow-card-float`) over default Tailwind `rounded-md` / `rounded-lg` / `shadow-sm` so curvature and depth follow the active theme. For glass navigation use `backdrop-filter: var(--backdrop-nav)` in CSS.
